...
mod tests {
    ...
    #[test]
    fn simplify_nested_elements_skips_detached_nodes() {
        let contents = r#"
        <div class="page">
            <div id="outer">
                <div id="middle">
        ...
        // Old implementation would panic when processing both `middle` and `inner`
        // because the parent of `middle` is removed when `inner` is lifted.
        let panicked = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let only_sel = page
                .select("div, section")
                .select(":is(div, section) > :is(div, section):only-child");
            for node in only_sel.nodes().iter().rev() {
                // This unwrap was the source of the production abort.
        ...
        .is_err();
        assert!(panicked, "expected old simplify_nested_elements to panic");

        // Patched implementation must no longer panic and should still flatten wrappers.
        simplify_nested_elements(&page);
        assert_eq!(page.select("#inner").length(), 1);
        assert_eq!(page.select("#middle").length(), 0);
        assert_eq!(page.select("#outer").length(), 0);
    }
}
